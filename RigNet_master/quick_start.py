...
def create_single_data(mesh_filaname):
    ...
    if not os.path.exists(mesh_filaname.replace('_remesh.obj', '_normalized.binvox')):
        os.system("./RigNet_master/binvox -d 88 -pb " + mesh_filaname.replace("_remesh.obj", "_normalized.obj"))
...
def run(input_folder,bandwidth,threshold,filename):
    ...
    jointNet.to(device)
    jointNet.eval()
    jointNet_checkpoint = torch.load('./RigNet_master/checkpoints/gcn_meanshift/model_best.pth.tar', map_location=torch.device('cpu'))
    ...
    rootNet.to(device)
    rootNet.eval()
    rootNet_checkpoint = torch.load('./RigNet_master/checkpoints/rootnet/model_best.pth.tar', map_location=torch.device('cpu'))
    ...
    boneNet.to(device)
    boneNet.eval()
    boneNet_checkpoint = torch.load('./RigNet_master/checkpoints/bonenet/model_best.pth.tar', map_location=torch.device('cpu'))
    ...
    skinNet_checkpoint = torch.load('./RigNet_master/checkpoints/skinnet/model_best.pth.tar', map_location=torch.device('cpu'))
    ...
    model_id, bandwidth, threshold = filename, bandwidth, threshold

    # create data used for inferece
    print("creating data for model ID {:s}".format(model_id))
    mesh_filename = os.path.join(input_folder, '{:s}_remesh.obj'.format(model_id))#this is where the format of the obj fil is collected
    print(mesh_filename)
...
if __name__ == '__main__':
    ...
    parser.add_argument('-o', '--obj_file', type=str, required=True)
    ...
    model_id, bandwidth, threshold = args.obj_file , None, 1e-5
...
'''if __name__ == '__main__':
    parser = argparse.ArgumentParser()
    parser.add_argument('-i', '--input_path', type=str, required=True)
    parser.add_argument('-b', '--bandwith', type=any, default=None, required=True)
    parser.add_argument('-t', '--threshold', type=float, default=1e-5, required=True)
    parser.add_argument('-o', '--obj_file', type=str, required=True)
    args = parser.parse_args()
    run(args.input_path, args.bandwith, args.threshold, args.obj_file)'''
